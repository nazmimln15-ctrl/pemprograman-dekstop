...
// kirim ke data base 
   String sql = "INSERT INTO db_mahasiswa (nama, umur) VALUES (?, ?)";
  ...
  String sql = "SELECT id, nama, umur FROM db_mahasiswa ORDER BY id";
  // konek ke database
  try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
  PreparedStatement ps = conn.prepareStatement(sql);
...
// update ke database
   String sql = "UPDATE db_mahasiswa SET nama = ?, umur = ? WHERE id = ?";
   // konek ke database
   try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
        PreparedStatement ps = conn.prepareStatement(sql)) {
  ...
  int id = Integer.parseInt(model.getValueAt(r, 0).toString());
   ...
   String sql = "DELETE FROM db_mahasiswa WHERE id = ?";
   try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASS);
        PreparedStatement ps = conn.prepareStatement(sql)) {
